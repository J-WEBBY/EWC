...
const RISK_COLORS: Record<RiskLevel, { bg: string; border: string; text: string; dot: string }> = {
  ...
  high:     { bg: 'rgba(217,119,6,0.07)',  border: 'rgba(217,119,6,0.25)',  text: '#D97706', dot: '#D97706' },
...
const REDLINE_STATUS_STYLE: Record<RedlineRule['status'], { bg: string; border: string; text: string; label: string }> = {
  ...
  active:     { bg: 'rgba(138,108,255,0.06)', border: '#EBE5FF',              text: '#6D28D9', label: 'Monitoring' },
  resolved:   { bg: 'rgba(5,150,105,0.06)',  border: 'rgba(5,150,105,0.20)', text: '#059669', label: 'Resolved' },
  suppressed: { bg: 'rgba(110,102,136,0.06)', border: '#EBE5FF',             text: '#8B84A0', label: 'Suppressed' },
};
...
function RiskGauge({ score, level }: { score: number; level: RiskLevel }) {
  ...
  return (
    <svg width={130} height={130} viewBox="0 0 130 130">
      ...
      />
      <text x={65} y={60} textAnchor="middle" fill="#1A1035" fontSize={28} fontWeight={900} fontFamily="inherit">
        {score}
      </text>
      <text x={65} y={76} textAnchor="middle" fill="#8B84A0" fontSize={10} fontFamily="inherit">
        RISK SCORE
      </text>
    </svg>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>
          {meta.label}
        </span>
        <span style={{
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
        <span style={{ fontSize: 32, fontWeight: 900, color: '#1A1035', lineHeight: 1, letterSpacing: '-0.03em' }}>
          {score.score}
        </span>
        <span style={{
          fontSize: 11, fontWeight: 700,
          color: deltaPos ? '#DC2626' : score.delta < 0 ? '#059669' : '#8B84A0',
        }}>
          {deltaStr}
        </span>
      </div>
      ...
      {/* Bar */}
      <div style={{ height: 4, borderRadius: 2, background: 'rgba(138,108,255,0.08)', overflow: 'hidden' }}>
        <div style={{
      ...
      <span style={{ fontSize: 10, color: '#6E6688' }}>{score.top_factor}</span>
    </div>
  );
}
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          background: rule.severity === 'critical' ? '#DC2626' : '#D97706',
        }} />

        {/* Code */}
        <span style={{ fontSize: 9, fontWeight: 700, color: '#8B84A0', width: 44, flexShrink: 0 }}>{rule.code}</span>

        {/* Title */}
        <span style={{ flex: 1, fontSize: 12, fontWeight: 700, color: rule.enabled ? '#1A1035' : '#8B84A0' }}>
          {rule.title}
        </span>
        ...
        {rule.trigger_count > 0 && (
          <span style={{ fontSize: 10, color: '#6E6688', flexShrink: 0 }}>
        ...
        <button
          ...
          style={{
            ...
            background: rule.enabled ? ACCENT : '#D5CCFF',
        ...
        {/* Expand arrow */}
        <svg width={12} height={12} viewBox="0 0 12 12" style={{ transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', color: '#8B84A0', flexShrink: 0 }}>
        ...
        {expanded && (
          ...
          >
            <div style={{ paddingBottom: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <p style={{ fontSize: 12, color: '#524D66', lineHeight: 1.6 }}>{rule.description}</p>
              <div style={{ display: 'flex', gap: 16 }}>
                <div>
                  <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>Category</span>
                  <p style={{ fontSize: 11, color: '#1A1035', fontWeight: 700, marginTop: 2, textTransform: 'capitalize' }}>{rule.category}</p>
                </div>
                <div>
                  <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>Severity</span>
                  <p style={{ fontSize: 11, color: rule.severity === 'critical' ? '#DC2626' : '#D97706', fontWeight: 700, marginTop: 2, textTransform: 'capitalize' }}>{rule.severity}</p>
                </div>
                {rule.last_triggered && (
                  <div>
                    <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>Last Triggered</span>
                    <p style={{ fontSize: 11, color: '#1A1035', fontWeight: 700, marginTop: 2 }}>{fmtDateTime(rule.last_triggered)}</p>
                  </div>
                )}
              </div>
              <div style={{ background: 'rgba(138,108,255,0.05)', border: '1px solid #EBE5FF', borderRadius: 8, padding: '8px 12px' }}>
                <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>Trigger Condition</span>
                <p style={{ fontFamily: 'monospace', fontSize: 11, color: '#524D66', marginTop: 4 }}>{rule.trigger_condition}</p>
              </div>
            </div>
          </motion.div>
        )}
  ...
  return (
    <motion.div layout style={{ borderBottom: '1px solid #EBE5FF' }}>
      ...
      >
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: '#1A1035', width: 100, flexShrink: 0 }}>{fmtDate(verdict.date)}</span>
        <span style={{ flex: 1, fontSize: 11, color: '#524D66', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {verdict.brief}
        </span>
        <span style={{
          fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 6,
        ...
        <span style={{ fontSize: 20, fontWeight: 900, color: '#1A1035', width: 36, textAlign: 'right', flexShrink: 0 }}>{verdict.overall_score}</span>
        <svg width={12} height={12} viewBox="0 0 12 12" style={{ transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', color: '#8B84A0', flexShrink: 0 }}>
          <path d="M2 4l4 4 4-4" stroke="currentColor" strokeWidth={1.5} fill="none" strokeLinecap="round" />
        </svg>
      </div>
          ...
          >
            <div style={{ paddingBottom: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', gap: 24 }}>
                <div>
                  <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>Confidence</span>
                  <p style={{ fontSize: 13, fontWeight: 900, color: '#1A1035' }}>{verdict.confidence}%</p>
                </div>
                <div>
                  <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>Signals Reviewed</span>
                  <p style={{ fontSize: 13, fontWeight: 900, color: '#1A1035' }}>{verdict.signals_reviewed}</p>
                </div>
                <div>
                  <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>Redlines</span>
                  <p style={{ fontSize: 13, fontWeight: 900, color: verdict.redlines_triggered > 0 ? '#DC2626' : '#059669' }}>{verdict.redlines_triggered}</p>
                </div>
                <div>
                  <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>Generated</span>
                  <p style={{ fontSize: 11, fontWeight: 700, color: '#1A1035', textTransform: 'capitalize' }}>{verdict.generated_by}</p>
                </div>
              </div>
              <div>
                <span style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>Key Risks</span>
                <ul style={{ margin: '6px 0 0', paddingLeft: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {verdict.key_risks.map((r, i) => (
                    <li key={i} style={{ fontSize: 11, color: '#524D66', display: 'flex', alignItems: 'center', gap: 6 }}>
    ...
    return (
      <div style={{ minHeight: '100vh', backgroundColor: '#FAF7F2', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 12, color: '#8B84A0' }}>Running assessment…</p>
        </div>
      ...
      <main style={{ paddingLeft: 240, minHeight: '100vh' }}>
        ...
        <div style={{ padding: '40px 40px 0', borderBottom: '1px solid #EBE5FF' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', paddingBottom: 24 }}>
            <div>
              <p style={{ fontSize: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.28em', color: '#8B84A0', marginBottom: 6 }}>
                Intelligence
              </p>
              <h1 style={{ fontSize: 38, fontWeight: 900, letterSpacing: '-0.035em', color: '#1A1035', lineHeight: 1 }}>
                Judgement Engine
              </h1>
            ...
            {(['brief', 'redlines', 'history'] as Tab[]).map(t => (
              <button
                ...
                style={{
                  ...
                  color: tab === t ? accentColor : '#8B84A0',
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="brief" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ padding: '32px 40px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                  ...
                  <div style={{ border: '1px solid #EBE5FF', borderRadius: 20, padding: '28px 28px', display: 'flex', gap: 24, alignItems: 'center' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: 22, fontWeight: 900, color: '#1A1035', letterSpacing: '-0.03em', textTransform: 'capitalize', lineHeight: 1.2 }}>
                        {today.overall_level} Risk
                      </p>
                      <p style={{ fontSize: 11, color: '#6E6688', marginTop: 6 }}>
                        Confidence: <strong style={{ color: '#1A1035' }}>{today.confidence}%</strong>
                      </p>
                      <p style={{ fontSize: 11, color: '#6E6688', marginTop: 4 }}>
                        {today.signals_reviewed} signals reviewed
                      </p>
                      <p style={{ fontSize: 11, color: '#6E6688', marginTop: 4 }}>
                        Assessed: <strong style={{ color: '#1A1035' }}>{fmtTime(data?.last_assessed ?? today.date)}</strong>
                      </p>
                      <p style={{ fontSize: 10, color: '#8B84A0', marginTop: 2 }}>
                  ...
                  <div style={{ border: '1px solid #EBE5FF', borderRadius: 20, padding: '24px 28px' }}>
                    ...
                    <p style={{ fontSize: 13, color: '#524D66', lineHeight: 1.7 }}>{today.brief}</p>
                  </div>

                  {/* Key risks */}
                  <div style={{ border: '1px solid #EBE5FF', borderRadius: 20, padding: '24px 28px' }}>
                    <p style={{ fontSize: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.28em', color: '#8B84A0', marginBottom: 14 }}>Key Risks</p>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {today.key_risks.map((risk, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                          <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#D97706', flexShrink: 0, marginTop: 4 }} />
                          <span style={{ fontSize: 12, color: '#524D66', lineHeight: 1.5 }}>{risk}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                  ...
                  {/* Category scores */}
                  <div>
                    <p style={{ fontSize: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.28em', color: '#8B84A0', marginBottom: 14 }}>Risk by Category</p>
                  ...
                  {/* Recommendations */}
                  <div style={{ border: '1px solid #EBE5FF', borderRadius: 20, padding: '24px 28px' }}>
                    <p style={{ fontSize: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.28em', color: '#8B84A0', marginBottom: 14 }}>Recommendations</p>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {today.recommendations.map((rec, i) => (
                        <div key={i} style={{
                          ...
                          <span style={{ fontSize: 12, color: '#524D66', lineHeight: 1.5 }}>{rec}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {history.length > 0 && (
                    <div style={{ border: '1px solid #EBE5FF', borderRadius: 20, padding: '24px 28px' }}>
                      <p style={{ fontSize: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.28em', color: '#8B84A0', marginBottom: 12 }}>7-Day Risk Trend</p>
                      <ScoreSparkline history={[today, ...history.slice(0, 6)]} />
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                        <span style={{ fontSize: 9, color: '#8B84A0' }}>7 days ago</span>
                        <span style={{ fontSize: 9, color: '#8B84A0' }}>Today</span>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 32 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {redlines.filter(r => r.status === 'triggered').map(r => (
                          <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            <span style={{ fontSize: 11, fontWeight: 700, color: '#1A1035' }}>{r.title}</span>
                          </div>
                        ))}
                      </div>
                      <p style={{ fontSize: 10, color: '#6E6688', marginTop: 12 }}>
                  ...
                  <div style={{ border: '1px solid #EBE5FF', borderRadius: 16, padding: '20px 20px' }}>
                    <p style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0', marginBottom: 10 }}>
                      What are Redlines?
                    </p>
                    <p style={{ fontSize: 12, color: '#524D66', lineHeight: 1.65 }}>
                    ...
                    <p style={{ fontSize: 12, color: '#524D66', lineHeight: 1.65, marginTop: 8 }}>
                  ...
                  <div style={{ border: '1px solid #EBE5FF', borderRadius: 16, padding: '20px 20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    {[
                      ...
                      <div key={stat.label}>
                        <p style={{ fontSize: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>{stat.label}</p>
                        <p style={{ fontSize: 24, fontWeight: 900, color: '#1A1035', letterSpacing: '-0.03em', marginTop: 2 }}>{stat.value}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {tab === 'history' && (
            <motion.div key="history" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ padding: '32px 40px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 32 }}>
                <div>
                  <p style={{ fontSize: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.28em', color: '#8B84A0', marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  {today && (
                    <div style={{ border: '1px solid #EBE5FF', borderRadius: 16, padding: '20px 20px' }}>
                      <p style={{ fontSize: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0', marginBottom: 12 }}>7-Day Trend</p>
                      <ScoreSparkline history={[today, ...history.slice(0, 6)]} />
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
                        <span style={{ fontSize: 9, color: '#8B84A0' }}>6 days ago</span>
                        <span style={{ fontSize: 9, color: '#8B84A0' }}>Today</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ border: '1px solid #EBE5FF', borderRadius: 16, padding: '20px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                    <p style={{ fontSize: 8, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.2em', color: '#8B84A0' }}>7-Day Averages</p>
                    {today && (() => {
                      ...
                      return (
                        <>
                          {[
                            ...
                            <div key={m.label}>
                              <p style={{ fontSize: 9, color: '#8B84A0', fontWeight: 600 }}>{m.label}</p>
                              <p style={{ fontSize: 22, fontWeight: 900, color: '#1A1035', letterSpacing: '-0.03em' }}>{m.value}{m.unit}</p>
                            </div>
                          ))}
                        </>
                      );
