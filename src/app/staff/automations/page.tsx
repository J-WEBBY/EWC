    ...
    >
      <div className="p-5">
        <div className="flex items-start gap-4">
          {/* Icon */}
          <div className="w-9 h-9 rounded-lg bg-[#FAF9F5] border border-[#EBE5FF] flex items-center justify-center flex-shrink-0">
            <AutoIcon size={15} className="text-[#524D66]" />
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2.5 flex-wrap mb-1">
                  <h3 className="text-[13px] font-medium text-[#1A1035]">{automation.name}</h3>
                  <span className="text-[10px] uppercase tracking-[0.1em] text-[#6E6688]">
                ...
                <p className="text-[12px] text-[#6E6688] leading-relaxed">{automation.description}</p>
              </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  onClick={handleRun}
                  disabled={running}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium bg-[#FAF9F5] border border-[#EBE5FF] text-[#524D66] hover:bg-white/[0.09] transition-colors disabled:opacity-40"
                >
                  {running ? (
                    <motion.div
                ...
                <button
                  ...
                  style={{ backgroundColor: automation.is_active ? '#8B84A0' : 'rgba(0,0,0,0.06)' }}
                  title={automation.is_active ? 'Disable' : 'Enable'}
                >
                  <motion.div
                    className="absolute top-0.5 w-4 h-4 rounded-full"
                    style={{ backgroundColor: automation.is_active ? '#1A1035' : '#6E6688' }}
            ...
            <button
              ...
              className="flex items-center gap-1 mt-2 text-[11px] text-[#6E6688] hover:text-[#6E6688] transition-colors"
            >
        ...
        <AnimatePresence>
          {expanded && (
            ...
            >
              <div className="mt-4 pt-4 border-t border-[#EBE5FF] grid grid-cols-3 gap-4">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.15em] text-[#6E6688] mb-1">Trigger</p>
                  <p className="text-[12px] text-[#524D66]">{automation.trigger_description}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-[0.15em] text-[#6E6688] mb-1">Type</p>
                  <p className="text-[12px] text-[#524D66] capitalize">{automation.trigger_type}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-[0.15em] text-[#6E6688] mb-1">Status</p>
                  <p className="text-[12px] text-[#524D66]">
...
function RunRow({ run }: { run: AutomationRun }) {
  ...
  return (
    <div className="flex items-center gap-4 py-3 border-b border-[#EBE5FF] last:border-0">
      <SIcon size={13} className="text-[#6E6688] flex-shrink-0" />
      <div className="flex-1 min-w-0">
        <p className="text-[13px] text-[#524D66] truncate">{run.automation_name}</p>
        <p className="text-[11px] text-[#6E6688] mt-0.5 truncate">{run.summary}</p>
      </div>
      <div className="text-right flex-shrink-0">
        <p className="text-[11px] text-[#6E6688]">{relativeTime(run.started_at)}</p>
        <p className="text-[10px] text-[#6E6688] mt-0.5 capitalize">{run.triggered_by}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed right-0 top-0 h-full z-50 w-[400px] flex flex-col bg-[#FAF7F2] border-l border-[#EBE5FF]"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#EBE5FF]">
          <div>
            <h2 className="text-[14px] font-semibold text-[#1A1035]">{aiName}</h2>
            <p className="text-[11px] text-[#6E6688] mt-0.5">Automation command</p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg flex items-center justify-center text-[#6E6688] hover:text-[#524D66] hover:bg-[#FAF7F2] transition-all"
          >
            <X size={14} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div
                ...
                style={
                  msg.role === 'user'
                    ? { backgroundColor: '#F5F2EB', color: '#1A1035', border: '1px solid #D5CCFF' }
                    : { backgroundColor: 'transparent', color: '#524D66', border: '1px solid #EBE5FF' }
                }
              >
                {msg.text}
              </div>
          ...
          {sending && (
            <div className="flex justify-start">
              <div className="px-4 py-3 rounded-xl bg-white border border-[#EBE5FF]">
        ...
        {messages.length <= 1 && (
          <div className="px-4 pb-3">
            <p className="text-[10px] uppercase tracking-[0.15em] text-[#8B84A0] mb-2">Try saying…</p>
            <div className="space-y-1.5">
              {examples.map(ex => (
                <button
                  key={ex}
                  onClick={() => send(ex)}
                  className="w-full text-left text-[11px] text-[#6E6688] px-3 py-2 rounded-lg bg-white border border-[#EBE5FF] hover:bg-[#FAF7F2] hover:text-[#524D66] transition-all truncate"
                >
                  {ex}
                </button>
              ))}
        ...
        {/* Input */}
        <div className="px-4 pb-4 pt-2 border-t border-[#EBE5FF]">
          <div className="flex gap-2">
            <input
              className="flex-1 bg-[#FAF9F5] border border-[#EBE5FF] rounded-lg px-3.5 py-2.5 text-[13px] text-[#1A1035] placeholder-white/20 outline-none focus:border-[#D5CCFF] transition-colors"
            ...
            <button
              ...
              className="w-9 h-9 rounded-lg flex items-center justify-center bg-[#8A6CFF] text-[#1A1035] hover:bg-[#8A6CFF]/10 transition-colors disabled:opacity-30 flex-shrink-0"
            >
              <Send size={13} />
            </button>
          </div>
      ...
      <AnimatePresence>
        {runNotice && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -16 }}
            className="fixed top-14 left-1/2 -translate-x-1/2 z-50 px-4 py-2.5 rounded-lg text-[13px] font-medium flex items-center gap-2 bg-white border border-white/[0.12] text-[#1A1035] backdrop-blur-sm"
          >
            <Play size={11} className="text-[#524D66]" />
            {runNotice}
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <main className="px-8 py-10 max-w-5xl mx-auto">
          ...
          >
            <div>
              ...
              <h1 className="text-[26px] font-semibold tracking-tight text-[#1A1035]">Automation Control</h1>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg text-[13px] font-medium bg-[#8A6CFF] text-[#1A1035] hover:bg-[#8A6CFF]/10 transition-colors"
              >
          ...
          {/* Tab bar */}
          <div className="flex items-center gap-1 mb-6 border-b border-[#EBE5FF]">
            {tabItems.map(t => (
              <button
                key={t.key}
                ...
                style={{
                  ...
                  color: tab === t.key ? '#1A1035' : '#6E6688',
                  fontWeight: tab === t.key ? 500 : 400,
                }}
              >
                {t.label}
                {t.count > 0 && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-white/[0.07] text-[#6E6688] min-w-[18px] text-center">
                    {t.count}
                  </span>
                )}
              </button>
          ...
          <AnimatePresence mode="wait">
            {tab === 'history' ? (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="bg-[#F0ECFF] border border-[#EBE5FF] rounded-xl px-5 py-2"
              >
                {runs.length === 0 ? (
                  <div className="py-10 text-center">
                    <Clock size={20} className="mx-auto mb-3 text-[#8B84A0]" />
                    <p className="text-[13px] text-[#6E6688]">No automation runs logged yet</p>
                  </div>
                ) : (
              ...
              >
                {displayed.length === 0 ? (
                  <div className="py-12 text-center bg-[#F0ECFF] border border-[#EBE5FF] rounded-xl">
                    <Radio size={20} className="mx-auto mb-3 text-[#8B84A0]" />
                    <p className="text-[13px] text-[#6E6688]">No active automations</p>
                    <button onClick={() => setTab('all')} className="mt-2 text-[12px] text-[#6E6688] hover:text-[#524D66] transition-colors">
