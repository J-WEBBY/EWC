...
const SOURCE_CONFIG: Record<TimelineSource, { label: string; color: string; bg: string; Icon: React.ElementType }> = {
  voice_komal: { label: 'VOICE · KOMAL', color: '#7C3AED', bg: 'rgba(124,58,237,0.07)',  Icon: Mic },
  agent_aria:  { label: 'AGENT · ARIA',  color: '#0D9488', bg: 'rgba(13,148,136,0.07)',  Icon: Bot },
  agent_orion: { label: 'AGENT · ORION', color: '#D97706', bg: 'rgba(217,119,6,0.07)',   Icon: Bot },
  agent_ewc:   { label: 'AGENT · EWC',   color: '#6D28D9', bg: 'rgba(109,40,217,0.07)',  Icon: Bot },
  ...
  sms_out:     { label: 'SMS',           color: '#6E6688', bg: 'rgba(110,102,136,0.05)',  Icon: MessageSquare },
  sms_in:      { label: 'SMS',           color: '#8B84A0', bg: 'rgba(139,132,160,0.05)', Icon: MessageSquare },
  email_out:   { label: 'EMAIL',         color: '#6E6688', bg: 'rgba(110,102,136,0.05)', Icon: Mail },
  email_in:    { label: 'EMAIL',         color: '#8B84A0', bg: 'rgba(139,132,160,0.05)', Icon: Mail },
};
...
function avatarColor(name: string) {
  const p = ['#7C3AED','#0284C7','#D97706','#0D9488','#DC2626','#059669','#EC4899','#6D28D9'];
...
function PatientRow({ patient, selected, onClick }: { patient: PatientSummary; selected: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      onMouseEnter={e => { if (!selected) (e.currentTarget as HTMLButtonElement).style.backgroundColor = 'rgba(138,108,255,0.04)'; }}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[12px] font-medium truncate" style={{ color: selected ? '#1A1035' : '#524D66' }}>{patient.full_name}</p>
        <p className="text-[9px] truncate mt-0.5" style={{ color: '#8B84A0' }}>{patient.last_treatment ?? 'No treatments'}</p>
      </div>
      <p className="text-[9px] flex-shrink-0" style={{ color: '#8B84A0' }}>
...
function TranscriptViewer({ transcript }: { transcript: { role: 'komal' | 'patient'; text: string }[] }) {
  return (
    <div className="mt-3 rounded-xl overflow-hidden" style={{ border: '1px solid #D5CCFF', backgroundColor: 'rgba(124,58,237,0.03)' }}>
      <div className="px-3 py-1.5 flex items-center gap-2" style={{ borderBottom: '1px solid #EBE5FF', backgroundColor: 'rgba(124,58,237,0.05)' }}>
        <Mic className="w-2.5 h-2.5" style={{ color: '#7C3AED' }} />
        <p className="text-[9px] uppercase tracking-[0.12em] font-semibold" style={{ color: '#7C3AED' }}>Call Transcript</p>
      </div>
        ...
        {transcript.map((line, i) => (
          <div key={i} className={`flex gap-2 ${line.role === 'patient' ? 'flex-row-reverse' : ''}`}>
            <div className="w-4 h-4 rounded-full flex items-center justify-center text-[7px] font-bold flex-shrink-0 mt-0.5"
              style={{ backgroundColor: line.role === 'komal' ? 'rgba(124,58,237,0.15)' : '#EBE5FF', color: line.role === 'komal' ? '#7C3AED' : '#6E6688' }}>
              {line.role === 'komal' ? 'K' : 'P'}
            </div>
            <div className="px-2.5 py-1.5 rounded-lg" style={{ maxWidth: '85%',
              backgroundColor: line.role === 'komal' ? 'rgba(124,58,237,0.06)' : 'white',
              border: '1px solid #EBE5FF' }}>
              <p className="text-[10px] leading-relaxed" style={{ color: line.role === 'komal' ? '#524D66' : '#6E6688' }}>{line.text}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="p-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <p className="text-[9px]" style={{ color: '#8B84A0' }}>{fmtTime(item.timestamp)}</p>
            {item.is_expandable && (
              <button onClick={onToggle} className="p-0.5 rounded" style={{ color: '#8B84A0' }}>
                {expanded ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
              </button>
            )}
          </div>
        </div>
        <p className="mt-2 text-[11px] leading-relaxed" style={{ color: '#6E6688' }}>
    ...
    <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
      ...
      {!isOut && (
        <div className="w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-bold flex-shrink-0 mr-2 mt-1"
          style={{ backgroundColor: '#EBE5FF', color: '#8B84A0' }}>P</div>
      )}
      <div style={{ maxWidth: '78%' }}>
        <div className={`flex items-center gap-1.5 mb-1 ${isOut ? 'justify-end' : 'justify-start'}`}>
          <span className="text-[8px] uppercase tracking-[0.08em]" style={{ color: '#8B84A0' }}>
            {isOut ? `${isEmail ? 'Email' : 'SMS'} · ${String(item.metadata?.sent_by ?? 'Staff')}` : `${isEmail ? 'Email' : 'SMS'} · Received`}
          </span>
          <span className="text-[8px]" style={{ color: '#8B84A0' }}>{fmtTime(item.timestamp)}</span>
        </div>
        <div className="px-3 py-2.5 rounded-2xl text-[11px] leading-relaxed"
          style={{
            backgroundColor: isOut ? 'rgba(138,108,255,0.08)' : 'white',
            color:           isOut ? '#1A1035' : '#524D66',
            border:          `1px solid ${isOut ? '#D5CCFF' : '#EBE5FF'}`,
        ...
        <div className={`flex mt-1 ${isOut ? 'justify-end' : 'justify-start'}`}>
          {isOut ? <ArrowUpRight className="w-2.5 h-2.5" style={{ color: '#D5CCFF' }} />
                 : <ArrowDownLeft className="w-2.5 h-2.5" style={{ color: '#D5CCFF' }} />}
        </div>
      </div>
      {isOut && (
        <div className="w-5 h-5 rounded-full flex items-center justify-center text-[7px] font-bold flex-shrink-0 ml-2 mt-1"
          style={{ backgroundColor: '#EBE5FF', color: '#6E6688' }}>EWC</div>
      )}
    </motion.div>
  );
}
...
function EmptyState({ label }: { label: string }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-3">
      <MessageCircle className="w-10 h-10" style={{ color: '#D5CCFF' }} />
      <p className="text-[12px]" style={{ color: '#8B84A0' }}>{label}</p>
    </div>
  );
}
...
export default function BridgePage() {
  ...
  const selColor = selectedPatient ? avatarColor(selectedPatient.full_name) : '#8A6CFF';
    ...
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: '#FAF7F2', paddingLeft: 'var(--nav-w, 240px)' }}>
      ...
      <div className="w-[248px] flex-shrink-0 flex flex-col overflow-hidden" style={{ borderRight: '1px solid #EBE5FF' }}>
        <div className="px-4 pt-5 pb-3" style={{ borderBottom: '1px solid #EBE5FF' }}>
          <p className="text-[8px] uppercase tracking-[0.28em] font-semibold mb-3" style={{ color: '#8B84A0' }}>
            Patients
            {patients.length > 0 && <span className="ml-2 px-1.5 py-0.5 rounded-md text-[8px]" style={{ backgroundColor: 'rgba(138,108,255,0.08)', color: '#8B84A0' }}>{patients.length}</span>}
          </p>
          <div className="relative">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3 h-3" style={{ color: '#8B84A0' }} />
            <input
              className="w-full pl-7 pr-3 py-2 rounded-lg text-[11px] focus:outline-none"
              style={{ backgroundColor: 'white', border: '1px solid #EBE5FF', color: '#1A1035' }}
        ...
        <div className="flex-1 overflow-y-auto py-1">
          {loading ? (
            <div className="flex items-center justify-center py-12"><Loader2 className="w-4 h-4 animate-spin" style={{ color: '#8B84A0' }} /></div>
          ) : filteredPatients.length === 0 ? (
            <p className="text-center text-[11px] py-8" style={{ color: '#8B84A0' }}>No patients found</p>
      ...
      <div className="flex-1 flex flex-col overflow-hidden" style={{ borderRight: '1px solid #EBE5FF' }}>
        ...
        <div className="px-5 py-3.5 flex items-center justify-between flex-shrink-0" style={{ borderBottom: '1px solid #EBE5FF' }}>
          <div className="flex items-center gap-3">
            <div className="px-2 py-0.5 rounded-md" style={{ backgroundColor: 'rgba(138,108,255,0.08)', border: '1px solid #EBE5FF' }}>
              <span className="text-[8px] uppercase tracking-[0.16em] font-semibold" style={{ color: '#8B84A0' }}>Section B</span>
            </div>
            <span className="text-[13px] font-semibold" style={{ color: '#1A1035' }}>Interaction Log</span>
            {selectedPatient && <span className="text-[13px]" style={{ color: '#8B84A0' }}>· {selectedPatient.full_name}</span>}
          </div>
          {selectedPatient && (
            <button
              ...
              className="p-1.5 rounded-lg transition-colors" style={{ color: '#8B84A0' }}
              onMouseEnter={e => (e.currentTarget.style.color = '#6E6688')}
          ...
          <div className="px-5 py-3 flex items-center gap-3 flex-shrink-0" style={{ borderBottom: '1px solid #EBE5FF', backgroundColor: `${selColor}05` }}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-0.5">
                {selectedPatient.phone && <span className="flex items-center gap-1 text-[10px]" style={{ color: '#6E6688' }}><Phone className="w-2.5 h-2.5" />{selectedPatient.phone}</span>}
                {selectedPatient.email && <span className="flex items-center gap-1 text-[10px]" style={{ color: '#6E6688' }}><Mail className="w-2.5 h-2.5" />{selectedPatient.email}</span>}
              </div>
            </div>
            <div className="text-right flex-shrink-0">
              {selectedPatient.last_treatment && <p className="text-[10px] font-medium" style={{ color: '#524D66' }}>{selectedPatient.last_treatment}</p>}
              <p className="text-[9px] mt-0.5" style={{ color: '#8B84A0' }}>{selectedPatient.interaction_count} interactions</p>
            </div>
          </div>
        )}
          ...
          <div className="px-5 py-2.5 flex items-center gap-1.5 overflow-x-auto flex-shrink-0" style={{ borderBottom: '1px solid #EBE5FF' }}>
            {(Object.keys(FILTER_MATCH) as TimelineFilter[]).map(f => (
              <button key={f} onClick={() => setTimelineFilter(f)}
                ...
                style={{
                  backgroundColor: timelineFilter === f ? 'rgba(138,108,255,0.12)' : 'transparent',
                  color:           timelineFilter === f ? '#6D28D9' : '#8B84A0',
                  border:          timelineFilter === f ? '1px solid #D5CCFF' : '1px solid transparent',
                }}>
                {f === 'all' ? 'All' : f}
                {filterCounts[f] > 0 && <span className="px-1 rounded text-[7px]" style={{ backgroundColor: timelineFilter === f ? 'rgba(109,40,217,0.12)' : 'rgba(138,108,255,0.06)', color: timelineFilter === f ? '#6D28D9' : '#8B84A0' }}>{filterCounts[f]}</span>}
              </button>
            ))}
          </div>
        )}
        ...
        {!selectedPatient ? (
          ...
          <div className="flex-1 flex items-center justify-center"><Loader2 className="w-5 h-5 animate-spin" style={{ color: '#8B84A0' }} /></div>
              ...
              {groupedTimeline.map(group => (
                <div key={group.label}>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-[9px] uppercase tracking-[0.14em] font-medium px-2" style={{ color: '#8B84A0' }}>{group.label}</span>
                    <div className="flex-1 h-px" style={{ backgroundColor: '#EBE5FF' }} />
                  </div>
      ...
      <div className="w-[308px] flex-shrink-0 flex flex-col overflow-hidden">
        <div className="px-5 py-3.5 flex items-center gap-3 flex-shrink-0" style={{ borderBottom: '1px solid #EBE5FF' }}>
          <div className="px-2 py-0.5 rounded-md" style={{ backgroundColor: 'rgba(138,108,255,0.08)', border: '1px solid #EBE5FF' }}>
            <span className="text-[8px] uppercase tracking-[0.16em] font-semibold" style={{ color: '#8B84A0' }}>Section A</span>
          </div>
        ...
        {!selectedPatient ? (
          ...
          <div className="flex-1 flex flex-col overflow-y-auto">
            <div className="p-5 space-y-5">
              {/* TO */}
              <div>
                <p className="text-[8px] uppercase tracking-[0.28em] font-semibold mb-2" style={{ color: '#8B84A0' }}>To</p>
                <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl" style={{ border: '1px solid #EBE5FF', backgroundColor: 'transparent' }}>
                  ...
                  <div>
                    ...
                    <p className="text-[10px] mt-0.5" style={{ color: '#8B84A0' }}>{selectedPatient.phone ?? selectedPatient.email ?? 'No contact details'}</p>
                  </div>
                </div>
              </div>
              ...
              {/* CHANNEL */}
              <div>
                <p className="text-[8px] uppercase tracking-[0.28em] font-semibold mb-2" style={{ color: '#8B84A0' }}>Channel</p>
                <div className="flex gap-2">
                  {(['sms', 'email', 'whatsapp'] as SendChannel[]).map(ch => (
                    <button key={ch} onClick={() => ch !== 'whatsapp' && setCompose(c => ({ ...c, channel: ch, body: '' }))}
                      ...
                      style={{
                        backgroundColor: compose.channel === ch ? 'rgba(138,108,255,0.10)' : 'transparent',
                        color:           compose.channel === ch ? '#6D28D9' : '#8B84A0',
                        border:          compose.channel === ch ? '1px solid #D5CCFF' : '1px solid #EBE5FF',
              ...
              {/* PURPOSE */}
              <div>
                <p className="text-[8px] uppercase tracking-[0.28em] font-semibold mb-2" style={{ color: '#8B84A0' }}>Purpose</p>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[8px] uppercase tracking-[0.28em] font-semibold" style={{ color: '#8B84A0' }}>Message</p>
                  {compose.channel === 'sms' && (
                    <p className="text-[9px] tabular-nums" style={{ color: compose.body.length > 160 ? '#DC2626' : '#8B84A0' }}>
                ...
                <textarea
                  ...
                  style={{ backgroundColor: 'white', border: '1px solid #EBE5FF', color: '#1A1035', minHeight: compose.channel === 'email' ? '120px' : '80px' }}
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleAIDraft} disabled={drafting}
                  ...
                  style={{ backgroundColor: 'rgba(138,108,255,0.06)', border: '1px solid #EBE5FF', color: drafting ? '#8B84A0' : '#6D28D9' }}
                  onMouseEnter={e => { if (!drafting) (e.currentTarget as HTMLButtonElement).style.borderColor = '#D5CCFF'; }}
                ...
                <button onClick={handleSend} disabled={sending || !compose.body.trim()}
                  ...
                  style={{
                    backgroundColor: sendSuccess ? 'rgba(5,150,105,0.10)' : (compose.body.trim() ? 'rgba(138,108,255,0.10)' : 'transparent'),
                    border:          sendSuccess ? '1px solid rgba(5,150,105,0.30)' : (compose.body.trim() ? '1px solid #D5CCFF' : '1px solid #EBE5FF'),
                    color:           sendSuccess ? '#059669' : (compose.body.trim() ? '#6D28D9' : '#8B84A0'),
              ...
              {timeline.filter(i => ['sms_out', 'email_out'].includes(i.source)).length > 0 && (
                <div>
                  <p className="text-[8px] uppercase tracking-[0.28em] font-semibold mb-3" style={{ color: '#8B84A0' }}>Recently Sent</p>
                      ...
                      <div key={item.id} className="px-3 py-2.5 rounded-xl" style={{ border: '1px solid #EBE5FF', backgroundColor: 'transparent' }}>
                        <div className="flex items-center justify-between mb-1">
                          <div className="flex items-center gap-1.5">
                            {item.source === 'email_out' ? <Mail className="w-2.5 h-2.5" style={{ color: '#8B84A0' }} /> : <MessageSquare className="w-2.5 h-2.5" style={{ color: '#8B84A0' }} />}
                            <span className="text-[9px] font-semibold uppercase tracking-[0.08em]" style={{ color: '#8B84A0' }}>{item.source === 'email_out' ? 'Email' : 'SMS'}</span>
                            {item.metadata?.sent_by && <span className="text-[9px]" style={{ color: '#8B84A0' }}>· {String(item.metadata.sent_by)}</span>}
                          </div>
                          <span className="text-[8px]" style={{ color: '#8B84A0' }}>{fmtTime(item.timestamp)}</span>
                        </div>
                        <p className="text-[10px] leading-relaxed line-clamp-2" style={{ color: '#6E6688' }}>{item.body}</p>
                      </div>
                    ))}
                  </div>
                </div>
