      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-2.5 mb-1 flex-wrap">
            <span className="text-[22px] font-black tracking-[-0.025em] text-[#1A1035] leading-none">
              {profile.displayName}
            </span>
            {agent.is_catch_all && (
            ...
            <span className="text-[9px] font-semibold uppercase tracking-[0.14em] px-2 py-0.5 rounded-full bg-[#F5F2EB] text-[#6E6688]">
              {profile.focus}
            </span>
          </div>
          ...
          <p className="text-[12px] text-[#6E6688] leading-relaxed mb-3">{profile.tagline}</p>

          {/* Capability chips */}
          <div className="flex flex-wrap gap-1.5">
            {profile.capabilities.map(cap => (
              <span key={cap}
                className="text-[10px] px-2 py-0.5 rounded-md text-[#524D66] font-medium"
          ...
          <div className="flex items-center gap-4 mt-3">
            <span className="text-[11px] text-[#8B84A0]">{agent.total_signals_handled} signals handled</span>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2.5 flex-wrap mb-1">
                  ...
                  <span className="text-[10px] uppercase tracking-[0.1em] text-[#6E6688]">{categoryLabel(automation.category)}</span>
                  <span className="text-[10px] uppercase tracking-[0.1em] text-[#6E6688]">{automation.trigger_type}</span>
                </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  onClick={handleRun}
                  disabled={running}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium bg-[#FAF9F5] border border-[#EBE5FF] text-[#524D66] hover:bg-white transition-colors disabled:opacity-40"
                >
                  {running ? (
                    <motion.div className="w-3 h-3 rounded-full border border-[#524D66]/40 border-t-transparent"
            ...
            <button
              ...
              className="flex items-center gap-1 mt-2 text-[11px] text-[#8B84A0] hover:text-[#6E6688] transition-colors"
            >
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
              ...
              <div className="mt-4 pt-4 border-t border-[#EBE5FF] grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-[#524D66]">{automation.is_active ? 'Active' : 'Inactive'}</p>
                </div>
              </div>
            </motion.div>
          )}
      ...
      <AnimatePresence>
        {autoRunNotice && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 px-5 py-2.5 rounded-xl bg-white border border-[#EBE5FF] text-[12px] text-[#1A1035] shadow-lg"
          >
            {autoRunNotice}
          </motion.div>
        )}
      ...
      <div className="min-h-screen px-8 py-10">
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[11px] uppercase tracking-[0.18em] text-[#6E6688] mb-2">Intelligence Layer</p>
              <h1 className="text-[26px] font-semibold tracking-tight text-[#1A1035]">Agents</h1>
            ...
            <button
              ...
              disabled={refreshing}
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-[13px] text-[#6E6688] bg-white border border-[#EBE5FF] hover:border-[#D5CCFF] transition-colors"
            >
            ...
            <motion.div key="agents" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                {[
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] uppercase tracking-[0.15em] text-[#6E6688] font-medium">{card.label}</span>
                      <card.icon size={14} className="text-[#6E6688]" />
                    </div>
                    <p className="text-[28px] font-semibold tracking-tight text-[#1A1035] leading-none">{card.value}</p>
                  </motion.div>
                ))}
              </div>
              ...
              {/* Agent list */}
              {agents.length === 0 ? (
                <div className="bg-white border border-[#EBE5FF] rounded-xl py-16 text-center">
                  <Bot size={24} className="mx-auto mb-3 text-[#8B84A0]" />
                  <p className="text-[13px] text-[#6E6688]">No agents configured</p>
                  <p className="text-[11px] text-[#8B84A0] mt-1">Run migration 014 to seed the three agents.</p>
                </div>
              ) : (
