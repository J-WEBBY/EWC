...
const FALLBACK: StaffProfile = {
  ...
  aiName: 'Aria', brandColor: '#D8A600', logoUrl: null,
...
function UtilBar({ pct, status }: { pct: number; status: CorporateAccount['status'] }) {
  const color = status === 'at_risk' ? '#DC2626' : pct >= 70 ? '#059669' : pct >= 40 ? '#D8A600' : '#2563EB';
  return (
    <div style={{ height: 4, background: 'rgba(0,88,230,0.08)', borderRadius: 2, overflow: 'hidden' }}>
      ...
      <div onClick={() => setExpanded(e => !e)} style={{ padding: '16px 0', display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 100px 1fr 90px', gap: 12, alignItems: 'center', cursor: 'pointer' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: '#181D23', margin: 0 }}>{account.company_name}</p>
          ...
          <p style={{ fontSize: 10, color: '#96989B', margin: 0 }}>{account.industry} · {account.employee_count} employees · {account.primary_contact.name}</p>
        </div>
        <div>
          <p style={{ fontSize: 18, fontWeight: 900, color: '#181D23', letterSpacing: '-0.03em', margin: 0 }}>{fmtCurrency(account.annual_value)}</p>
          <p style={{ fontSize: 10, color: '#96989B', margin: 0 }}>ARR</p>
        </div>
        <div>
          <UtilBar pct={account.utilisation_pct} status={account.status} />
          <p style={{ fontSize: 10, color: '#5A6475', marginTop: 4 }}>{account.utilisation_pct}% utilised · {account.treatments_ytd} treatments YTD</p>
        </div>
        <div>
          <p style={{ fontSize: 11, fontWeight: 700, color: days < 30 ? '#DC2626' : days < 90 ? '#D8A600' : '#181D23', margin: 0 }}>
            {days < 0 ? 'EXPIRED' : days + 'd'}
          </p>
          <p style={{ fontSize: 10, color: '#96989B', margin: 0 }}>{fmtDate(account.contract_end)}</p>
        </div>
        <div>
...
function PipelineFunnel({ accounts, accentColor }: { accounts: CorporateAccount[]; accentColor: string }) {
  const stages: { key: AccountStatus; label: string; color: string }[] = [
    ...
    { key: 'renewal_due',  label: 'Renewal Due',   color: '#D8A600' },
      ...
      <div className="p-5 space-y-3">
            ...
            <div key={s.key}>
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[11px] font-medium" style={{ color: '#181D23' }}>{s.label}</p>
                ...
                <div className="flex items-center gap-3">
                  <p className="text-[11px] font-semibold" style={{ color: '#3D4451' }}>£{stageArr.toLocaleString('en-GB')}</p>
                  <p className="text-[10px] w-8 text-right" style={{ color: '#96989B' }}>{pct}%</p>
                </div>
              </div>
        ...
        <div className="pt-3 mt-1 grid grid-cols-3 gap-3" style={{ borderTop: '1px solid #D4E2FF' }}>
          {[
            ...
            { label: 'Total Accounts', value: String(accounts.length), color: '#3D4451' },
          ].map(k => (
...
function RenewalCalendar({ accounts }: { accounts: CorporateAccount[] }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid #D4E2FF' }}>
      ...
      <div className="p-2">
        {upcoming.length === 0 ? (
          <p className="px-3 py-4 text-[12px] text-center" style={{ color: '#96989B' }}>No upcoming renewals</p>
        ) : (
          upcoming.map(a => {
            const days = Math.round((new Date(a.contract_end).getTime() - Date.now()) / 86400000);
            const urgColor = days < 0 ? '#DC2626' : days <= 30 ? '#DC2626' : days <= 90 ? '#D8A600' : '#059669';
            ...
            return (
              <div key={a.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] font-semibold truncate" style={{ color: '#181D23' }}>{a.company_name}</p>
                  <p className="text-[10px]" style={{ color: '#96989B' }}>
      ...
      <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #D4E2FF' }}>
        ...
        {totalOutstanding > 0 && (
          <div className="text-right">
            ...
            <p className="text-[9px]" style={{ color: '#96989B' }}>total overdue</p>
          </div>
        )}
      </div>
      <div className="p-2">
        {withInvoice.length === 0 ? (
          ...
          withInvoice.map(a => (
            <div key={a.id} className="flex items-center justify-between px-3 py-2.5 rounded-xl"
              ...
              <div>
                <p className="text-[12px] font-semibold" style={{ color: '#181D23' }}>{a.company_name}</p>
                <p className="text-[10px]" style={{ color: '#96989B' }}>{a.primary_contact.name} · {a.primary_contact.email}</p>
              </div>
      ...
      <main style={{ paddingLeft: 240, minHeight: '100vh' }}>
        <div style={{ padding: '40px 40px 0', borderBottom: '1px solid #D4E2FF' }}>
          <div style={{ paddingBottom: 24 }}>
            ...
            <p style={{ fontSize: 13, color: '#3D4451', marginTop: 6 }}>B2B employer wellness packages — AI account briefs, renewal pipeline, utilisation tracking</p>
          </div>
          {/* View tabs */}
          <div style={{ display: 'flex', gap: 0, marginTop: 4 }}>
            {[{ key: 'accounts' as const, label: `Accounts (${accounts.length})` }, { key: 'pipeline' as const, label: 'Pipeline & Renewals' }].map(t => (
              <button key={t.key} onClick={() => setView(t.key)} style={{
                ...
                cursor: 'pointer', color: view === t.key ? accentColor : '#96989B',
        ...
        {stats && (
          <div style={{ display: 'flex', borderBottom: '1px solid #D4E2FF' }}>
            {[
              ...
              { label: 'Renewals <30d',   value: String(stats.renewal_due_30d),    color: '#D8A600' },
